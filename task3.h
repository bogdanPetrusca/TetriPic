...
void task3()
{
	...
	for(i=0;i<header2->height;i++)
		for(j=0;j<header2->width;j++)
			a[i][j]=*black_pixel;
	for(i=header2->height-40;i<header2->height;i++)
		for(j=0;j<header2->width;j++)
			a[i][j]=*white_pixel;
	...
	int white_line,cont=0;
	for(row=0;row<header2->height-40;row++)
	{
		for(column=0;column<header2->width;column++)
			if(a[row][column].blue == 255 && a[row][column].green == 255 && a[row][column].red == 255)
				cont++;
		if(cont == header2->width)
		{
			white_line=row;
			break;
		}
	}
	if(white_line!=header2->height-40)
	{
		for(row=white_line;row<header2->height;row++)
			for(column=0;column<header2->width;column++)
				a[row][column]=*white_pixel;
		for(row=white_line;row<header2->height-40;row++)
			for(column=0;column<header2->width;column++)
				a[row][column]=*black_pixel;
	}
	free(black_pixel);
